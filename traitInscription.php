    ...
    if(isset($_POST['pseudo']) AND isset($_POST['nom']) AND isset($_POST['prenom']) AND isset($_POST['mpass'])){
        ...
        if(isset($_FILES['profile'])){
            ...
            if($verifPseudo->fetch() > 0){
                ...
                $insertUser = $db->prepare("INSERT INTO membre(pseudo,prenom,nom, profil, mpass) 
                VALUES ('$pseudo', '$prenom', '$nom', '$profile', '$mpass')");
                if($insertUser->execute()){
                    
                    $insertUserSec = $db->prepare("INSERT INTO membresecondaire(pseudo,prenom,nom, profil, mpass) 
                        VALUES ('$pseudo', '$prenom', '$nom', '$profile', '$mpass')");
